...
def main():
    parser = argparse.ArgumentParser(description="Wrapper to run generate_banking_tts_batched.py sequentially in fresh processes to prevent memory leak")
    ...
    cmd = [
        "python3", "-u", "scratch/generate_banking_tts_batched.py",
    ...
    print(f"Starting banking wrapper script. Running command: {' '.join(cmd)}")
    
    run_idx = 1
    while True:
        print(f"\n==============================================")
        print(f"Banking Wrapper Run #{run_idx} at {time.strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"==============================================")
