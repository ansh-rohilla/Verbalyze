...
DATA_DIR = WORKSPACE_DIR / "scratch" / "domain_specific_data_v4"
LOCAL_AUDIO_BASE = WORKSPACE_DIR / "scratch" / "domain_specific_data_v4_audio"
S3_BUCKET = "vighnesh-voice-data-2026"

VOICE_CONFIGS = {
    "as": {"female": "female, young adult, moderate pitch", "male": "male, middle-aged, low pitch"},
    "bn": {"female": "female, young adult, moderate pitch", "male": "male, middle-aged, low pitch"},
    "mr": {"female": "female, young adult, moderate pitch", "male": "male, middle-aged, low pitch"},
    "or": {"female": "female, young adult, moderate pitch", "male": "male, middle-aged, low pitch"},
    "en": {"female": "female, young adult, moderate pitch", "male": "male, middle-aged, low pitch"}
}
...
DIGIT_MAPS_NATIVE = {
    "as": {"0": "শূন্য", "1": "এক", "2": "দুই", "3": "তিমি", "4": "চাৰি", "5": "পাঁচ", "6": "ছয়", "7": "সাত", "8": "আঠ", "9": "ন"},
...
def process_language_batched(lang: str, model, resume: bool = True, steps: int = 15, max_records: int = 0, batch_size: int = 4, range_start: int = 0, range_end: int = 0, output_suffix: str = ""):
    print(f"\n==============================================")
    print(f"Processing Language (BATCHED): {lang.upper()}")
    print(f"==============================================")
    
    jsonl_file = DATA_DIR / lang / f"transcriptsv1_{lang}.jsonl"
    if not jsonl_file.exists():
        print(f"Error: transcripts JSONL file {jsonl_file} does not exist.")
        return False
    ...
    output_jsonl_file = DATA_DIR / lang / f"transcriptsv1_{lang}_with_audio{suffix_str}.jsonl"
    
    # Load completed records from this partition file
    completed_records = {}
    ...
    # Load global completed IDs
    main_jsonl_file = DATA_DIR / lang / f"transcriptsv1_{lang}_with_audio.jsonl"
    global_completed_ids = set(completed_records.keys())
    if main_jsonl_file.exists():
        with open(main_jsonl_file, "r", encoding="utf-8") as f:
            for line in f:
                if line.strip():
                    try:
                        rec = json.loads(line)
                        global_completed_ids.add(rec["id"])
                    except Exception:
                        pass
    ...
    for idx, row in enumerate(all_records):
        ...
        if native_text:
            currency_keywords = [
                "টকা", "ৰূপ", "টাকা", "রুপये", "रुपये", "रुपे", "రూపాయలు", "ರೂಪಾಯಿ", "rupees", "rupee", "rs"
            ]
            is_currency = any(kw in native_text.lower() for kw in currency_keywords)
        is_sample = (idx % 5 == 0)
        native_required = is_currency or is_sample
        
        has_native = (lang != "en") and bool(native_text) and native_required
        ...
        if resume and row_id in global_completed_ids and tts_exists and native_exists:
            continue
            
        row["tts_clean_text"] = post_process_digits(row["tts_reading"], lang, is_native=False)
        row["tts_audio_path"] = f"s3://{S3_BUCKET}/audio/{lang}/{row_id}_tts.wav"
        
        if has_native:
            row["native_clean_text"] = post_process_digits(native_text, lang, is_native=True)
            row["native_audio_path"] = f"s3://{S3_BUCKET}/audio/{lang}/{row_id}_native.wav"
        else:
            row.pop("native_tts_reading", None)
    ...
    print(f"Pending records (before slicing): {len(pending_records)}")
    
    if range_start > 0 or range_end > 0:
        start_idx = range_start
        end_idx = range_end if range_end > 0 else len(pending_records)
        print(f"Slicing records: index {start_idx} to {end_idx} (out of {len(pending_records)})")
        pending_records = pending_records[start_idx:end_idx]
        
    print(f"Pending records to generate in this run: {len(pending_records)}")
    
    if len(pending_records) == 0:
        print(f"Language {lang.upper()} partition is already fully complete!")
        return True
...
def main():
    ...
    for lang in langs:
        completed = process_language_batched(
            lang, model, resume=True, steps=args.steps, 
