...
def run_telephony_gate_verification():
    ...
    if not clean_audio or not os.path.exists(clean_audio):
        print("[FAIL] Error: Synthesis failed to generate audio.")
        sys.exit(1)

    clean_report = clean_engine.last_quality_report
    if clean_report is None:
        clean_report = clean_engine.scorer.evaluate(clean_audio, test_text)
        clean_engine.last_quality_report = clean_report

    print(f"[PASS] Clean Audio Generated: {clean_audio}")
    ...
    print(f"[PASS] Telephony Channel Applied:")
    ...
    print(f"   • Quality Gate Decision: {'[PASS] ACCEPTED' if telephony_report.passed else '[FAIL] REJECTED'}")
    ...
    fd, sim_dest = tempfile.mkstemp(suffix=".mp3")
    os.close(fd)
    ...
    print(f"[PASS] Native Telephony Engine generated audio at: {sim_out}")
    ...
    print(f"[PASS] SIP Inbound Handshake Successful:")
    ...
    print(f"[PASS] SIP Spoken Turn Handshake Successful:")
